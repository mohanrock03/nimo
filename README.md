...
- **Speech recognition = browser's Web Speech API.** Free, no key, no account. In Chrome the audio is sent to Google's speech service (Edge: Microsoft's), so it needs internet and is not private/offline. Works in Chrome and Edge on desktop. Not Firefox. Safari/iPhone and Android are unreliable for always-listening.
...
## Tests
`node tests/run-tests.js` - 57 automated tests (wake word, Tanglish/Tamil replies, and turn-taking with a fake mic/voice/clock: never speaks while the mic is open, waits for you to finish, echo guard, follow-up window, sleep, tap-to-interrupt, auto-restart).
